...
| **옵션 (product_option)** | 절대 금액을 가진 선택지. 옵션 재고는 **선택** — 넣으면 폼 재고와 두 층으로 같이 움직인다 (D-054) |
...
| 배송비 청구 | 2차금에서 | **1차금에서** (이연할 곳이 없다 · D-046) |
| 배송비 금액 | 폼에 정한 값, 없으면 셀러 기본값. 묶음에 폼이 여럿이면 최대값 (D-053) | 같다 |
| 재고 성격 | 모집 정원 | 실제 재고 |
...
**재고는 판매 폼 단위다.** 상품 단위가 아니다. 옵션에는 **선택적으로** 재고를 둘 수 있고,
그때는 폼 재고가 옵션 합계이며 확보 · 해제 · 확정 · 되돌림이 두 층을 같이 움직인다 (D-054).
...
| 환불 계좌 | **1차금 결제 전 필수.** 판매 유형을 가리지 않는다. 스냅샷 없이 마스터 한 벌 (D-057) |
...
| **셀러·구매자 겸업** | 명세에 없던 항목. 코드는 겸업 가능으로 구현돼 있다 (같은 `kakao_id` 가 두 테이블에 존재 가능). 카카오 앱 1개 사용으로 이 전제를 확정했다 — `decisions.md` D-019 |
