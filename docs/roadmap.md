...
- [x] 취소 가능 여부 판단 (정산 전 / EOB 시간대 / 이미 취소 중)
- [x] 부분 취소 세금 비율 계산
- [x] `Point3Client.refund(...)` + 409 코드별 분기
- [x] `resume` API 연동
- [x] **취소 대사 배치**
- [x] 정산 후 환불 접수 (`settled_manual`)
- [x] 목표수량 미달 시 자동 취소 (`shortfall_policy`) — EXTEND 는 연장 규칙 미확정으로 보류 (D-026)
- [x] 구매자 취소 API · 폼 단위 부분 취소 (D-025)
- [x] 셀러 결제 내역 · 결제 취소 (G10) — D-059. 구매자와 같은 엔진·같은 구간이다
- [x] 정산 후 환불 처리 (S14) — D-059. `manual_refunded_at` 으로 대기·완료를 가른다
- [ ] 실결제 검증 — 샌드박스가 없어 소액 실결제로만 확인할 수 있다
...
- [x] Outbox 테이블 + 릴레이 배치 (D-027)
- [x] 카카오 알림톡 연동 (SOLAPI) — 결제완료 템플릿 승인·연동 완료 (D-040). 나머지 템플릿은 승인되는 대로 파라미터만 추가하면 나간다
- [x] 수신번호 — **배송지 번호를 쓰기로 확정** (D-040, 09-15). 알림톡은 번호로만 갈 수 있고 카카오 계정 경로는 토큰 만료 때문에 접었다. 선물 주문은 수령인에게 가는 제약을 안고 간다
- [ ] 알림톡 실발송 켜기 — `SOLAPI_TEST_RECIPIENT` 를 걸고 소액 실결제로 릴레이·조립·수신번호까지 확인한 뒤 값을 지워 연다. SOLAPI 구간은 09-15 재확인됨
- [x] 재시도 상한 + DEAD 처리 — 8회
- [ ] 셀러 대시보드 집계 API
