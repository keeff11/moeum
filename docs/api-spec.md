...
| **20** 🆕 | `GET /admin/sellers` | 심사 신청자 목록 (운영자). 프록시가 외부 호출을 막는다 | — | 프록시 |
| **21** 🆕 | `GET /me/orders/in-progress` | 진행 중인 결제 목록. 잃어버린 orderToken 을 되찾는다 | **B6** | 필요 (구매자) |
| **22** 🆕 | `GET /me/orders` | 나의 구매 목록. 탭(전체·공동구매·단독판매) · 상태 배지 · 취소 가능 여부 · 낸 금액(D-060) | **B13** | 필요 (구매자) |
| **23** 🆕 | `GET /stores` | 셀러 리스트. 승인된 셀러의 공개 프로필 + 이름 검색 (D-061) | **셀러 리스트** | 불필요 |
...
| `stock` | 재고 5개 | 폼 전체. `stockMax - held - sold` |
| `options[].stock` 🆕 | 스티퍼 상한 | **선택한 옵션의 값을 쓴다** (D-054). 옵션 재고가 있으면 폼 재고와 옵션 재고 중 작은 쪽, 없으면 폼 재고와 같다 |
...
이 시점에 재고가 홀드되고 30분 타이머가 시작된다. 실패하면 B4 로 넘어가지 않는다.
...
| `seller.shippingFee` | — | ⚠️ **옵션이 아니라 폼 수준이다.** 주문 묶음당 1회. 폼에 정한 배송비가 있으면 그것, 없으면 셀러 기본값 (D-053) |
| `seller.freeShippingOver` | — | 이 금액 이상이면 배송비 면제. `null`이면 미적용 |

🆕 `options[].stock` — **옵션별 재고가 생겼다** (D-054). 다만 이 정적 응답에는 없고
`/availability` 의 `options[].stock` 으로 받는다 — 재고는 휘발성이라 캐시되는 응답에 섞지 않는다.
옵션 재고를 쓰지 않는 폼도 같은 필드를 준다(폼 재고와 같은 값). 화면은 늘 이 값을 스티퍼 상한으로 쓴다.

⚠️ `options[].shippingFee` — **옵션별 배송비는 없다.**
배송비는 **주문 묶음당 1회**다. 옵션마다 실어 보내면 옵션 수량만큼 배송비가 붙는 것으로
오해하기 쉽다. 옵션 총액에서도 배송비를 빼고 `optionAmount = deposit1 + deposit2` 로만 내려보낸다.
...
## 5-2. B5 · 환불 계좌 등록 🆕 (D-057)

### `GET · PUT /me/refund-account`

| 필드 | 화면 표시 | 비고 |
|---|---|---|
| `bank` | 은행 | **필수.** 화면의 선택지 문자열을 그대로 보낸다 (최대 30자) |
| `accountNo` | 계좌번호 | **필수.** 하이픈은 있어도 없어도 된다 — 서버가 숫자만 남겨 저장한다 |
| `holderName` | 예금주 | **필수.** 받는 사람 이름과 다를 수 있어 따로 받는다 |

```json
// PUT 요청
{ "bank": "국민은행", "accountNo": "1002-123-456789", "holderName": "홍길동" }

// 200 · GET 응답 (등록 전이면 본문 없이 204)
{ "bank": "국민은행", "accountNoMasked": "****6789", "holderName": "홍길동",
  "updatedAt": "2026-09-15T21:00:00" }
```

> 🆕 **`/pay` 는 환불 계좌가 없으면 400 (`REFUND_ACCOUNT_REQUIRED`) 이다.**
> 배송지와 같은 자리의 선행 조건이고, **공동구매·단독(재고) 판매를 가리지 않는다.**
> 정산이 끝난 뒤의 환불(S14 · B8-C3)은 결제 수단으로 되돌릴 수 없어, 그때 돈을 보낼
> 곳이 이 계좌밖에 없다.

**단일 계좌.** 계좌부가 아니라 덮어쓰기다. 배송지와 같은 모양이지만 **주문 시점
스냅샷을 뜨지 않는다** — 환불은 몇 주 뒤에 일어나고 그때 살아 있는 계좌로 보내야 한다.
...
> ⚠️ **청구액은 판매 유형에 따라 갈린다** (D-046).
> - **공동구매** — 배송비가 2차 결제로 이연되어 **B5 청구액은 옵션가뿐**이다.
>   와이어프레임의 "상품 32,000원 + 배송비 2,500원 = 총 34,500원"은 이 경우 갱신이 필요하다.
> - **단독 판매** — 2차금 단계가 없어서(domain.md 1절) 이연할 곳이 없다.
>   **배송비까지 1차금에서 받는다** — 와이어프레임 표기가 그대로 맞다.
>
> 화면에서 직접 더하지 말고 `POST /checkout-sessions` 응답의 **`firstPaymentAmount`** 를 쓴다.
> 이 값에 규칙이 반영돼 있고, 결제창에 뜨는 금액과 같다.
...
> 🆕 **`/pay` 는 환불 계좌가 없어도 400 (`REFUND_ACCOUNT_REQUIRED`) 이다** (D-057).
> B5 에서 배송지와 함께 받는 필수 항목이고 **판매 유형을 가리지 않는다.**
> 두 코드는 갈라서 다뤄야 한다 — 되돌릴 화면이 서로 다르다.
...
| `SALE_PAUSED` | 409 | 불가 | 일시중지 안내 (D-056. **마감 안내로 묶지 않는다** — 다시 열릴 판매다) |
...
- 배송지 · 환불 계좌 확인 → 없으면 `400 SHIPPING_ADDRESS_REQUIRED` · `400 REFUND_ACCOUNT_REQUIRED`
...
```
2차 결제 청구 총액 = SUM(주문 항목별 deposit2Amount) + 배송비 1회
```

⚠️ 배송비는 **묶음당 1회**다. 주문 항목 수·수량과 무관하게 한 번만 더한다.
폼에 정한 배송비가 있으면 그것, 없으면 셀러 기본값이고, 묶음에 폼이 여럿이면 가장 큰 값이다 (D-053).
...
| ③ | 옵션별 재고 관리 | ~~안 한다~~ → **한다 (D-054, 2026-09-14).** 옵션마다 `stock` 을 넣으면 옵션 재고, 비우면 폼 재고. `/availability` 의 `options[].stock` 이 스티퍼 상한 |
...
| ⑦ | shippingFee 정책 | ⚠️ **폼 단위 · 주문 묶음당 1회 (D-053).** 폼에 정한 값이 없으면 셀러 기본값. 묶음에 폼이 여럿이면 최대값. 옵션별이 아니다. 공동구매는 2차금, 단독 판매는 1차금에 합산 (D-046) |
...
6. ~~⚠️ `options[].stock` 제거~~ → 🆕 **`/availability` 의 `options[].stock` 을 스티퍼 상한으로** (D-054). 상품 정적 응답에는 여전히 없다
7. 🆕 **실패 응답의 `code` 세분화** — 재시도 가능/불가 구분
8. ⚠️ **`/release` 실패 대비 명시** — 서버 만료 배치가 최종 안전망
9. ⚠️ **`options[].shippingFee` · `options[].totalAmount` 제거** — 배송비는 묶음당 1회.
   옵션에는 `optionAmount`(= `deposit1 + deposit2`)만 두고, 배송비는 `seller.shippingFee` 로 받는다.
   🆕 그 값은 폼에 정한 배송비가 반영된 것이다 (D-053) — 셀러 페이지의 배송비와 다를 수 있다
