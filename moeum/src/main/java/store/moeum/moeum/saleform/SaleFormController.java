...
public class SaleFormController {
	...
	@Operation(summary = "판매 진행 현황",
			description = """
					...
					★ stages 는 화면의 단계 스트립 그대로다 — 공동구매는 모집중 · 마감 · 발주 ·
					  제작중 · 입고·2차금 · 발송 여섯 칸이고, 단독 판매는 모집·발주가 없어
					  결제 완료 · 배송 준비 중 · 발송 셋이다. 문구(label)도 그대로 쓰면 된다.
					★ 지금 단계(stage)는 살아 있는 주문 중 가장 덜 진행된 것이다.
					  한 건이라도 남아 있으면 판매가 그 단계를 벗어난 것이 아니다.
					★ 발주(ORDERED)는 stage 로 오지 않는다 — 셀러가 누르는 버튼이 하나라
					  발주와 제작중이 같은 전이의 두 칸이고, 두 칸의 건수도 같다.
					  그래서 칸 건수를 더하지 말고 totalOrders 를 쓴다.
